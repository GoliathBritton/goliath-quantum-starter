...
class GoliathQuantum:
    ...
    async def optimize_qubo(self, 
                           ...
                           algorithm: str = "qaoa",
                           enable_pouw: bool = True) -> OptimizationResponse:
    ...
    def get_dynex_statistics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "enabled": True,
                "network": self.dynex_api.network.value if self.dynex_api else "unknown",
                "wallet_configured": bool(self.dynex_api.wallet_address if self.dynex_api else None),
                "api_key_configured": bool(self.dynex_api.api_key if self.dynex_api else None)
            }
        except Exception as e:
    ...
    def _validate_optimization_request(self, request: OptimizationRequest):
        ...
        if request.data.size > self.max_qubits ** 2:
            raise ValueError(f"Data size {request.data.size} exceeds max qubits {self.max_qubits}")
    
    def _convert_to_optimization_problem(self, request: OptimizationRequest) -> OptimizationProblem:
        """Convert request to optimization problem"""
        problem_type = ProblemType.QUBO if request.problem_type == "qubo" else ProblemType.ML_CLASSIFICATION
    ...
    def _select_algorithm(self, algorithm_name: str, problem: OptimizationProblem) -> OptimizationAlgorithm:
