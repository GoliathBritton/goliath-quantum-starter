...
def optimize_qubo(variables: int, Q: List, constraints: List = None, objective: str = "maximize") -> dict:
    """
    ...
    if DYNEX_AVAILABLE:
        try:
            ...
            if isinstance(Q, list) and all(isinstance(x, (list, tuple)) and len(x) == 3 for x in Q):
                for i, j, v in Q:
...
class QuantumAdapter:
    ...
    def solve_qubo(self, variables: int, Q: List, constraints: List = None, objective: str = "maximize") -> dict:
    ...
    def set_backend(self, backend: str):
        ...
        if backend in self.available_backends:
            ...
            raise ValueError(f"Backend {backend} not available. Choose from {self.available_backends}")
