...
DIVISIONS = {
    "flyfox_ai": {
        ...
        "automations": ["lead_scoring", "sales_script", "openai_chat", "neuro_siphon", "q_mirrors", "sigma_graph"],
        "demo_workflows": [
            {
                "name": "AI Lead Scoring Demo",
                "description": "Score a lead using quantum/AI policy.",
                "run": lambda: AUTOMATIONS["lead_scoring"]({"lead_score": 0.9, "risk": 0.05})
            },
            {
    ...
    "goliath_trade": {
        ...
        "automations": ["quantum_optimize", "qubo_optimization", "sat_solver", "qboost_train", "qsvm_train", "data_poisoning", "dead_drop"],
        "demo_workflows": [
            {
                "name": "Quantum Portfolio Optimization",
                "description": "Optimize a portfolio using QUBO.",
                "run": lambda: AUTOMATIONS["quantum_optimize"](2, [(0,0,1.0),(1,1,-1.0),(0,1,0.5)])
            },
            {
                "name": "Data Poisoning (Reality Distortion)",
                "description": "Distort data using QNLP reality distortion.",
                "run": lambda: AUTOMATIONS["data_poisoning"]({"sales": 100, "risk": 0.2})
            },
            {
    ...
    "sigma_select": {
        ...
        "automations": ["lead_scoring", "openai_embedding", "sigma_graph", "data_poisoning"],
        "demo_workflows": [
            {
                "name": "Risk Scoring Demo",
                "description": "Score risk for an insurance lead.",
                "run": lambda: AUTOMATIONS["lead_scoring"]({"lead_score": 0.7, "risk": 0.3})
            },
            {
                "name": "Sigma Graph Org Chart",
                "description": "Build org chart and compute leverage scores.",
        ...
        "demo_workflows": [
            {
                ...
                "run": lambda: AUTOMATIONS["lead_scoring"]({"lead_score": 0.85, "risk": 0.15})
            },
            {
