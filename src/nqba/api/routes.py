...
from .models import (
    ...
    ModelType, InferenceMode, SystemMetricsResponse,
    ReversalReasoningRequest, ReversalReasoningResponse,
    QAOAOptimizationRequest, QAOAOptimizationResponse
)
...
@quantum_router.post("/reasoning", response_model=ReversalReasoningResponse)
async def reversal_reasoning(request: ReversalReasoningRequest) -> ReversalReasoningResponse:
    """Perform reversal reasoning using quantum-enhanced bidirectional processing"""
    try:
        logger.info(f"Processing reversal reasoning request: {request.premise[:50]}...")
        
        # Call the quantum reasoning algorithm
        ...
        performance_stats = reasoning_engine.get_performance_stats()
        
        return ReversalReasoningResponse(
            forward_reasoning=result.get('forward', ''),
            backward_reasoning=result.get('backward', ''),
            coherence_score=result.get('coherence', 0.0),
        ...
        logger.error(f"Error in reversal reasoning: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to process reversal reasoning: {str(e)}"
        )

@quantum_router.post("/optimization", response_model=QAOAOptimizationResponse)
async def qaoa_optimization(request: QAOAOptimizationRequest) -> QAOAOptimizationResponse:
    """Perform QAOA optimization for finance, energy, insurance, or general problems"""
    try:
        logger.info(f"Processing QAOA optimization request: {request.problem_type}")
        
        # Call the quantum optimization algorithm
        ...
        return QAOAOptimizationResponse(
            ...
            num_workers_used=result.get('num_workers_used', 1),
            performance_stats=performance_stats
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to process QAOA optimization: {str(e)}"
        )

@quantum_router.get("/reasoning/stats")
async def get_reasoning_stats() -> Dict[str, Any]:
    """Get performance statistics for the reasoning engine"""
    try:
        return reasoning_engine.get_performance_stats()
    except Exception as e:
        logger.error(f"Error getting reasoning stats: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get reasoning stats: {str(e)}"
        )

@quantum_router.get("/optimization/stats")
async def get_optimization_stats() -> Dict[str, Any]:
    """Get performance statistics for the optimization engine"""
    try:
        return qaoa_engine.get_performance_stats()
    except Exception as e:
