...
class IAMManager:
    ...
    def _initialize_role_permissions(self):
        ...
        self.role_permissions = {
            RoleLevel.OWNER: {
                Permission.ORG_CREATE, Permission.ORG_DELETE, Permission.ORG_UPDATE, Permission.ORG_VIEW,
                Permission.USER_CREATE, Permission.USER_DELETE, Permission.USER_UPDATE, Permission.USER_VIEW,
                Permission.API_KEY_CREATE, Permission.API_KEY_DELETE, Permission.API_KEY_UPDATE, Permission.API_KEY_VIEW,
                Permission.DATA_READ, Permission.DATA_WRITE, Permission.DATA_DELETE,
                Permission.QUANTUM_ACCESS, Permission.QUANTUM_ADMIN,
                Permission.BU_ACCESS, Permission.BU_ADMIN,
                Permission.MONITORING_VIEW, Permission.ANALYTICS_ACCESS
            },
            RoleLevel.ADMIN: {
                Permission.ORG_VIEW, Permission.ORG_UPDATE,
                Permission.USER_CREATE, Permission.USER_UPDATE, Permission.USER_VIEW,
                Permission.API_KEY_CREATE, Permission.API_KEY_UPDATE, Permission.API_KEY_VIEW,
    ...
    def create_organization(self, name: str, domain: str, owner_email: str, owner_username: str) -> Dict[str, str]:
    ...
    def create_user(
        ...
        for user in self.users.values():
            if user.email == email and user.org_id == org_id:
                raise ValueError(f"User {email} already exists in organization {org_id}")
        ...
        logger.info(f"Created user {email} in organization {org_id} with role {role.value}")
        return {
            "user_id": user_id,
            "org_id": org_id
        }
    ...
    def check_rate_limit(self, key_id: str) -> bool:
        ...
        tracker.requests_per_hour = [
            req_time for req_time in tracker.requests_per_hour
        ...
        if (len(tracker.requests_per_minute) >= api_key.rate_limit_per_minute or
            len(tracker.requests_per_hour) >= api_key.rate_limit_per_hour):
            return False
            ...
            "organizations": [
                {
                    ...
                    "name": org.name,
                    "domain": org.domain,
                    "user_count": sum(1 for user in self.users.values() if user.org_id == org.org_id)
                }
