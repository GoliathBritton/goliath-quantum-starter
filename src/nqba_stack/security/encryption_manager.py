...
class EncryptionManager:
    ...
    def create_tenant(self, tenant_id: str, encryption_level: EncryptionLevel = EncryptionLevel.ENHANCED) -> str:
        ...
        master_key_id = f"tenant_{tenant_id}_master_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        logger.info(f"Created tenant {tenant_id} with {encryption_level.value} encryption")
    ...
    def encrypt_data(self, data: Any, tenant_id: str, encryption_level: EncryptionLevel = None) -> Dict[str, Any]:
    ...
    def _get_or_create_data_key(self, tenant_id: str, encryption_level: EncryptionLevel) -> str:
        ...
        for key_id, key in self.encryption_keys.items():
            if (key.tenant_id == tenant_id and 
                key.level == encryption_level and 
                key.is_active):
                return key_id
    ...
    def _encrypt_dict(self, data: Dict[str, Any], key: EncryptionKey, config: TenantEncryptionConfig) -> Dict[str, Any]:
        ...
        for field_name, value in data.items():
            if field_name in config.pii_fields:
                ...
                encrypted_value = self.encrypt_field(str(value), config.tenant_id, field_name)
                encrypted_dict[field_name] = encrypted_value
            elif isinstance(value, (dict, list)):
                # Recursively encrypt nested structures
                encrypted_dict[field_name] = self.encrypt_data(value, config.tenant_id, key.level)
            else:
    ...
    def _decrypt_dict(self, encrypted_data: Dict[str, Any], key: EncryptionKey) -> Dict[str, Any]:
        ...
        for field_name, value in encrypted_data.items():
            if isinstance(value, str) and value.startswith('eyJ'):
                ...
                try:
                    decrypted_dict[field_name] = self.decrypt_field(value, key.tenant_id)
                except:
                ...
                decrypted_dict[field_name] = self.decrypt_data({"encrypted_data": value}, key.tenant_id)
            else:
                # Regular decryption
                decrypted_dict[field_name] = self._decrypt_string(value, key)
    ...
    def rotate_keys(self, tenant_id: str) -> bool:
        ...
        new_master_key_id = f"tenant_{tenant_id}_master_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def get_tenant_status(self, tenant_id: str) -> Dict[str, Any]:
        ...
        # Count active keys
        active_keys = sum(1 for key in self.encryption_keys.values() 
                         if key.tenant_id == tenant_id and key.is_active)
    ...
    def get_global_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_keys": active_keys,
            "tenants": [self.get_tenant_status(tenant_id) for tenant_id in self.tenant_configs.keys()]
        }
