...
class GridOptimizer:
        ...
        try:
            ...
            for i, demand in enumerate(demands):
                for j, supply in enumerate(supplies):
                    ...
                    distance_cost = self._calculate_distance_cost(
                        demand.location, supply.location
                    )
            ...
            if quantum_result.success:
                ...
                if solution is not None and len(solution) > 0:
                    ...
                    optimal_flows = self._extract_optimal_flows(
                        solution, demands, supplies
                    )

                    # Calculate metrics
                    total_cost = self._calculate_total_cost(
                        optimal_flows, demands, supplies
                    )
                    emissions_reduction = self._calculate_emissions_reduction(
                        optimal_flows, supplies
                    )
            ...
            logger.warning(
                f"Quantum grid optimization failed: {e}, using classical fallback"
            )
            return self._classical_grid_optimization(
                demands, supplies, grid_constraints
            )
    ...
    def _calculate_emissions_reduction(
        self, flows: Dict[str, float], supplies: List[EnergySupply]
    ) -> float:
            ...
            for flow_key, flow_amount in flows.items():
                ...
                if supply:
                    if supply.source_type in [
                        ...
                        emissions_factor = (
                            0.8 if supply.source_type == EnergySource.FOSSIL else 0.0
                        )
    ...
    def _calculate_reliability_score(
        ...
        try:
            ...
            supply_diversity = len(set(s.source_type for s in supplies)) / len(
                EnergySource
            )
        ...
        try:
            ...
            if quantum_result.success:
                ...
                if solution is not None and len(solution) > 0:
                    ...
                    forecast = self._extract_demand_forecast(
                        solution, n_hours, n_patterns, historical_patterns
                    )
            ...
            logger.warning(
                f"Quantum demand forecasting failed: {e}, using classical fallback"
            )
            return self._classical_demand_forecast(
                historical_demand, weather_data, forecast_horizon
            )
        ...
        try:
            ...
            for h in range(forecast_horizon):
                ...
                if h < len(weather_data):
                    ...
                    # Combine factors
                    predicted_demand = (
                        baseline_demand[hour_of_day] * temp_factor * hour_factor
                    )
    ...
    def _extract_historical_patterns(
        ...
        try:
            ...
            for hour in range(24):
                hour_data = historical_demand[historical_demand.index.hour == hour][
                    "demand"
                ]
                if len(hour_data) > 0:
...
class RenewableIntegration:
        ...
        try:
            ...
            if quantum_result.success:
                ...
                if solution is not None and len(solution) > 0:
                    ...
                    integration_plan = self._extract_integration_plan(
                        solution, renewable_sources, n_time_periods
                    )
                    ...
                    return {
                        ...
                        "utilization_rate": self._calculate_utilization_rate(
                            integration_plan, renewable_sources
                        ),
                    }
                else:
                    raise Exception("Invalid quantum solution")
            ...
            logger.warning(
                f"Quantum integration optimization failed: {e}, using classical fallback"
            )
            return self._classical_integration_optimization(
                renewable_sources, grid_capacity, storage_capacity
            )
        ...
        try:
            ...
            for i, source in enumerate(renewable_sources):
                ...
                for t in range(n_time_periods):
                    ...
                    if var_idx < len(solution) and solution[var_idx] == 1:
                        availability = self._get_time_availability(
    ...
    def _calculate_utilization_rate(
        self, integration_plan: Dict[str, float], renewable_sources: List[EnergySupply]
    ) -> float:
...
class StorageOptimizer:
    ...
    async def _quantum_storage_optimization(
        ...
        try:
                ...
                if solution is not None and len(solution) > 0:
                    ...
                    return {
                        ...
                        "total_storage_used": sum(
                            abs(v) for v in storage_schedule.values()
                        ),
            ...
            logger.warning(
                f"Quantum storage optimization failed: {e}, using classical fallback"
            )
            return self._classical_storage_optimization(
                demand_profile, renewable_profile, storage_capacity
            )
    ...
    def _extract_storage_schedule(
        self, solution: List[int], n_hours: int
    ) -> Dict[str, float]:
    ...
    def _calculate_grid_balance_improvement(
        ...
        try:
            ...
            original_imbalance = sum(
                abs(d - r) for d, r in zip(demand_profile, renewable_profile)
            )
            ...
            if original_imbalance > 0:
                improvement = (
                    (original_imbalance - new_imbalance) / original_imbalance
                ) * 100
