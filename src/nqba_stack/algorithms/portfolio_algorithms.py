...
class QuantumPortfolioOptimizer:
    ...
    async def optimize_portfolio(
        ...
        try:
            ...
            if strategy == OptimizationStrategy.MAXIMIZE_SHARPE:
                ...
                qubo_matrix = self._build_variance_qubo(
            ...
            try:
                ...
                if quantum_result.success:
                    weights = self._extract_weights_from_quantum(quantum_result, len(expected_returns))
            ...
            for j in range(n_assets):
                for b1 in range(num_bits):
                    for b2 in range(num_bits):
                        ...
                        weight1 = 2 ** b1
                        weight2 = 2 ** b2
                        Q[var1_idx, var2_idx] += lambda_risk * covariance_matrix.iloc[i, j] * weight1 * weight2
            ...
            for j in range(n_assets):
                for b1 in range(num_bits):
                    for b2 in range(num_bits):
                        ...
                        weight1 = 2 ** b1
                        weight2 = 2 ** b2
                        Q[var1_idx, var2_idx] = covariance_matrix.iloc[i, j] * weight1 * weight2
            ...
            for b1 in range(num_bits):
                ...
                for j in range(n_assets):
                    for b2 in range(num_bits):
                        var2_idx = j * num_bits + b2
                        weight2 = 2 ** b2
                        individual_risk += covariance_matrix.iloc[i, j] * weight1 * weight2
                
                # Penalty for deviation from target
                Q[var1_idx, var1_idx] += lambda_parity * (individual_risk - target_risk_contribution) ** 2
    ...
    def _extract_weights_from_quantum(
        ...
        try:
            ...
            if hasattr(quantum_result, 'solution'):
                ...
                logger.warning("Could not extract quantum solution, using random weights")
    ...
    def _calculate_portfolio_metrics(
        ...
        for i, symbol in enumerate(expected_returns.index):
            ...
            risk_contributions[symbol] = weights[i] * marginal_risk / portfolio_risk if portfolio_risk > 0 else 0
