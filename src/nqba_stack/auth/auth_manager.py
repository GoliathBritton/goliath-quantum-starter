...
class AuthManager:
    ...
    def create_user(
        ...
        is_valid, message = self.password_manager.validate_password_strength(
            user_data.password
        )
        if not is_valid:
    ...
    def authenticate_user(
        ...
        session = UserSession(
            ...
            + timedelta(days=1 if login_data.remember_me else 1),
        )
    ...
    def update_user(
        self, user_id: str, updates: UserUpdate, updater_roles: List[str]
    ) -> Tuple[bool, str]:
        """
    ...
    def get_user_by_id(
        self, user_id: str, requester_roles: List[str]
    ) -> Tuple[bool, str, Optional[User]]:
        """
        Get user by ID
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_roles": [
                role.name for role in self.rbac.roles.values() if role.is_system_role
            ],
        }
