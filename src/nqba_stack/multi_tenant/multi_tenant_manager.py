...
class MultiTenantManager:
    ...
    def _validate_resource_limits(self, resource_limits: Dict[str, float]):
        ...
        for resource_type, limit in resource_limits.items():
            ...
            if limit > self.available_resources[resource_type]:
                raise ValueError(
                    f"Resource limit exceeds available capacity: {resource_type}"
                )
    ...
    def _get_default_scaling_config(
    ...
    async def _reserve_resources(
        self, tenant_id: str, resource_limits: Dict[str, float]
    ):
    ...
    async def record_tenant_metrics(
        self, tenant_id: str, metrics_data: Dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            if (
                ...
                and self.tenants[tenant_id].scaling_policy != ScalingPolicy.MANUAL
            ):
        ...
        try:
            ...
            if metrics.cpu_utilization > scaling_config.get("scale_up_threshold", 0.8):
                await self._create_scaling_decision(
                    ...
                    scaling_config.get("scaling_factor", 1.5),
                )
            elif metrics.cpu_utilization < scaling_config.get(
                "scale_down_threshold", 0.3
            ):
                await self._create_scaling_decision(
                    tenant_id,
                    ...
                    "compute",
                    metrics.cpu_utilization,
                    1.0 / scaling_config.get("scaling_factor", 1.5),
                )

            # Check memory utilization scaling
            if metrics.memory_utilization > scaling_config.get(
                "scale_up_threshold", 0.8
            ):
                await self._create_scaling_decision(
                    tenant_id,
            ...
            if metrics.response_time > tenant.performance_thresholds.get(
                "response_time", 1.5
            ):
                await self._create_scaling_decision(
                    tenant_id,
    ...
    async def _create_scaling_decision(
        ...
        try:
            ...
            if (
                abs(recommended_allocation - current_allocation) / current_allocation
                < self.min_scaling_threshold
            ):
                return ""
            ...
            decision = ScalingDecision(
                ...
                cost_implications=self._calculate_scaling_cost(
            ...
            logger.info(
                f"Created scaling decision: {decision.decision_id} for tenant {tenant_id}"
            )
    ...
    def _get_current_resource_allocation(
        ...
        for allocation in self.resource_allocations.values():
            if (
                ...
                and allocation.resource_type.value == resource_type
            ):
                return allocation.allocated_amount
        return 0.0

    def _calculate_scaling_cost(
        self, resource_type: str, current: float, recommended: float
    ) -> float:
    ...
    async def _execute_scaling_decision(self, decision_id: str) -> bool:
        ...
        try:
            ...
            if decision.scaling_type == "scale_up":
                resource_needed = (
                    decision.recommended_allocation - decision.current_allocation
                )
                if not await self._check_resource_availability(
                    {decision.resource_type.value: resource_needed}
                ):
                    logger.warning(
                        f"Insufficient resources for scaling decision: {decision_id}"
                    )
                    return False

            # Update resource allocation
            await self._update_resource_allocation(
                tenant_id, decision.resource_type.value, decision.recommended_allocation
            )
    ...
    async def _update_resource_allocation(
        self, tenant_id: str, resource_type: str, new_amount: float
    ):
        ...
        try:
            ...
            if "response_time" in sla_requirements:
                ...
                if metrics.response_time > max_response_time:
                    violations.append(
                        f"Response time SLA violation: {metrics.response_time}s > {max_response_time}s"
                    )
            ...
            if "availability" in sla_requirements:
                ...
                if metrics.availability < min_availability:
                    violations.append(
                        f"Availability SLA violation: {metrics.availability} < {min_availability}"
                    )
            ...
            if "error_rate" in sla_requirements:
                ...
                if metrics.error_rate > max_error_rate:
                    violations.append(
                        f"Error rate SLA violation: {metrics.error_rate} > {max_error_rate}"
                    )
    ...
    async def _cleanup_old_metrics(self, tenant_id: str):
        ...
        try:
            ...
            # Remove old metrics
            self.tenant_metrics[tenant_id] = [
                m for m in self.tenant_metrics[tenant_id] if m.timestamp >= cutoff_time
            ]
        ...
        try:
            ...
            analytics = {
                ...
                "business_metrics": {
                    ...
                    "operations_per_second": np.mean(
                        [m.operations_per_second for m in recent_metrics]
                    ),
                    "revenue_per_hour": np.mean(
                        [m.revenue_per_hour for m in recent_metrics]
                    ),
                },
                "scaling_info": {
                    ...
                    "recent_scaling_decisions": len(
                        [
                            d
                            for d in self.scaling_decisions.values()
                            if d.tenant_id == tenant_id
                            and d.timestamp >= datetime.now() - timedelta(hours=24)
                        ]
                    ),
                },
            }
        ...
        try:
            ...
            for tenant_id in self.tenants:
                ...
                if metrics:
                    ...
                    if "response_time" in tenant.sla_requirements:
                        total_sla_checks += 1
                        if (
                            recent_metrics.response_time
                            > tenant.sla_requirements["response_time"]
                        ):
                            sla_violations += 1
                    ...
                    if "availability" in tenant.sla_requirements:
                        total_sla_checks += 1
                        if (
                            recent_metrics.availability
                            < tenant.sla_requirements["availability"]
                        ):
                            sla_violations += 1

            sla_compliance_rate = (
                (total_sla_checks - sla_violations) / max(total_sla_checks, 1) * 100
            )
            ...
            return {
                "system_overview": {
                    ...
                    "tenant_activation_rate": active_tenants
                    / max(total_tenants, 1)
                    * 100,
                },
                "resource_management": {
                ...
                "performance_monitoring": {
                    ...
                    "total_metrics_records": sum(
                        len(metrics) for metrics in self.tenant_metrics.values()
                    ),
                },
            }
    ...
    async def cleanup_old_data(self, max_age_hours: int = 168):
        ...
        try:
            ...
            for tenant_id in self.tenant_metrics:
                self.tenant_metrics[tenant_id] = [
                    m
                    for m in self.tenant_metrics[tenant_id]
            ...
            old_decisions = [
                d_id
                for d_id, decision in self.scaling_decisions.items()
