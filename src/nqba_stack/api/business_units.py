...
async def _run_energy_optimization(energy_data: List[Dict]) -> Dict[str, Any]:
    ...
    total_current_cost = sum(
        float(row.get("cost", 0)) for row in energy_data if row.get("cost")
    )
    ...
    optimized_monthly = [
        int(cost * (1 - optimization_factor)) for cost in current_monthly
    ]
...
async def apply_for_capital(
    application: dict, current_user: dict = Depends(JWTHandler().verify_token)
):
    """
...
async def _run_capital_assessment(application: dict) -> Dict[str, Any]:
    ...
    return {
        ...
        "risk_level": (
            ...
            else "Medium" if final_risk_score < 60 else "High"
        ),
...
async def get_insurance_quote(
    quote_request: dict, current_user: dict = Depends(JWTHandler().verify_token)
):
    """
