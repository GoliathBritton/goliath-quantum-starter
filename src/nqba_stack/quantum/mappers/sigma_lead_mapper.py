...
class SigmaLeadMapper(ProblemMapper):
    ...
    async def map_to_qubo(self, problem_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                    ...
                    for i2 in range(n_leads):
                        for j2, channel2 in enumerate(channels):
                            ...
                            if y_idx <= y2_idx:  # Avoid double counting
                                qubo_matrix[y_idx][y2_idx] += budget_penalty * channel_cost * channel_cost2
            ...
            logger.info(f"Sigma Lead Mapper: Mapped {n_leads} leads to {total_vars} QUBO variables")
            return result
    ...
    async def validate_input(self, problem_data: Dict[str, Any]) -> bool:
        ...
        for lead in leads:
            ...
            required_lead_fields = ["id", "engagement_score", "budget_score", "authority_score"]
    ...
    async def interpret_solution(self, solution: Dict[str, Any], problem_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            total_expected_revenue = sum(
                self._calculate_expected_revenue(lead)
                for lead in selected_leads
            )
            
            roi = (total_expected_revenue - total_cost) / total_cost if total_cost > 0 else 0
