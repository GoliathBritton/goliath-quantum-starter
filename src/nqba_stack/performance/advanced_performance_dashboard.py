...
class AdvancedPerformanceDashboard:
    ...
    async def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # Log update
                logger.debug(
                    f"Dashboard update {self.update_count} completed in {execution_time:.2f}s"
                )
    ...
    async def _calculate_trend(
        self, metric_name: str, metrics_data: List[Dict[str, Any]]
    ...
    async def _check_alerts(self):
        ...
        try:
            ...
            await self._check_metric_alert(
                "error_rate", self.system_health.error_rate_avg, "System Error Rate"
            )

            # Check CPU utilization alerts
            await self._check_metric_alert(
                "cpu_utilization", self.system_health.cpu_utilization, "CPU Utilization"
            )
    ...
    async def _check_metric_alert(
        self, metric_name: str, current_value: float, display_name: str
    ):
    ...
    async def record_performance_metrics(
        self, metric_type: str, metrics_data: Dict[str, Any]
    ):
    ...
    async def _cleanup_old_metrics(self, metric_type: str):
        ...
        try:
            ...
            self.performance_metrics[metric_type] = [
                m
                for m in self.performance_metrics[metric_type]
                if m.get("timestamp", datetime.min) >= cutoff_time
            ]
    ...
    async def _cleanup_old_business_metrics(self, tenant_id: str):
        ...
        try:
            ...
            self.business_metrics[tenant_id] = [
                m
                for m in self.business_metrics[tenant_id]
    ...
    async def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "system_health": (
                    self.system_health.__dict__ if self.system_health else None
                ),
                "alerts_summary": {
                ...
                "business_metrics": {
                    ...
                    "total_business_metrics": sum(
                        len(metrics) for metrics in self.business_metrics.values()
                    ),
                },
            }
