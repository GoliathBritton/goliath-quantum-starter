...
class AuditSchedule:
    ...
    company_name: str = (
        f"{BRANDING['goliath']['name']} | {BRANDING['flyfox']['name']} | {BRANDING['sigma_select']['name']}"
    )
...
class NQBAScheduledAudits:
    ...
    def _calculate_next_audit_date(
        self, frequency: AuditFrequency, current_date: datetime
    ) -> datetime:
    ...
    async def _send_audit_notifications(
        self, audit_schedule: AuditSchedule, assessment_result: AssessmentResult
    ):
    ...
    async def upgrade_subscription(
        self, company_id: str, new_tier: SubscriptionTier
    ) -> bool:
        ...
        self.audit_schedules[company_id].audit_frequency = new_tier_config[
            "audit_frequency"
        ]
    ...
    async def get_company_audit_history(self, company_id: str) -> List[AuditExecution]:
        ...
        return [
            execution
            for execution in self.audit_executions.values()
    ...
    async def get_subscription_stats(self) -> Dict[str, Any]:
        ...
        total_subscribers = len(
            [s for s in self.audit_schedules.values() if s.is_active]
        )
        tier_counts = {}
        revenue_by_tier = {}
