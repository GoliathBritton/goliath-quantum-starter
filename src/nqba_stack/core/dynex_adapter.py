...
class DynexAdapter:
        ...
        if config is None:
            ...
            if not api_key:
                ...
                if env != "production":
                    ...
                    raise ValueError(
                        "Dynex not configured. Set DYNEX_API_KEY environment variable."
                    )
            config = DynexConfig(
                api_key=api_key,
                ...
                default_annealing_time=getattr(
                    settings, "dynex_default_annealing_time", 100
                ),
    ...
    def solve_qubo(
        ...
        try:
            ...
            sampler = dynex.DynexSampler(
                mainnet=self.config.mainnet, description=description
            )
    ...
    def _build_lead_scoring_bqm(
        ...
        for idx, lead in enumerate(lead_data):
            ...
            if budget in ["high", "very high"] and pain_points in [
                "energy costs",
                "production delays",
            ]:
                bqm.add_interaction(f"budget_high_{idx}", f"pain_energy_{idx}", -20)

            if urgency in ["urgent", "very urgent"] and pain_points in [
                "energy costs",
                "production delays",
            ]:
    ...
    def solve_energy_optimization_qubo(
    ...
    def _build_energy_optimization_bqm(
        self, energy_data: Dict[str, Any]
    ) -> dimod.BinaryQuadraticModel:
