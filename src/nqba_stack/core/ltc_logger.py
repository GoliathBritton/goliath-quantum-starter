...
class LTCLogger:
    ...
    def log_operation(
        ...
        try:
            ...
            with self.lock:
                ...
                operation_data_json = json.dumps(operation_data, default=str)
                metadata_json = json.dumps(metadata, default=str) if metadata else None
                timestamp_str = ltc_operation.timestamp.isoformat()

                cursor.execute(
                    """
                    INSERT INTO ltc_operations 
                    (operation_id, operation_type, operation_data, thread_ref, timestamp, hash_chain, ipfs_reference, metadata)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (
                        str(operation_id),
                        ...
                        timestamp_str,
                        str(hash_chain),
                        None,  # ipfs_reference will be updated later if needed
                        metadata_json,
                    ),
                )
            ...
            return (
                f"fallback_{hashlib.md5(str(datetime.now()).encode()).hexdigest()[:8]}"
            )
            ...
            with self.lock:
                ...
                for row in rows:
                    ...
                    data_string = json.dumps(
                        json.loads(operation_data), sort_keys=True, default=str
                    )
                    calculated_hash = hashlib.sha256(
                        f"{verified_hash}{data_string}".encode()
                    ).hexdigest()
    ...
    def get_last_operation(self, operation_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.lock:
                ...
                cursor.execute(
                    """
                    SELECT operation_id, operation_type, operation_data, thread_ref, 
                           timestamp, hash_chain, ipfs_reference, metadata
                    FROM ltc_operations 
                ...
                if row:
                    return {
                        "operation_id": row[0],
                        "operation_type": row[1],
                        "operation_data": json.loads(row[2]) if row[2] else {},
                        "thread_ref": row[3],
                        "timestamp": row[4],
                        "hash_chain": row[5],
                        "ipfs_reference": row[6],
        ...
        try:
            with self.lock:
                ...
                return {
                    ...
                    "ipfs_backup_rate": (
                        (ipfs_backed_operations / total_operations * 100)
...
def log_operation(
    ...
    return ltc_logger.log_operation(
        operation_type, operation_data, thread_ref, metadata
    )
