...
class ConfigurationManager:
    ...
    def _load_configuration(self):
        ...
        self.services = {
            ...
            "ipfs": ServiceConfig(
                name="ipfs",
                enabled=bool(os.getenv("IPFS_PROJECT_ID") and os.getenv("IPFS_PROJECT_SECRET")),
                ...
                metadata={
                    ...
                    "gateway_url": os.getenv("IPFS_GATEWAY_URL", "https://gateway.pinata.cloud")
                }
            ),
            "llm": ServiceConfig(
                name="llm",
    ...
    def should_use_fallback(self, service_name: str) -> bool:
        ...
        if service.health_check_enabled and not self.health_status.get(service_name, True):
            return True
    ...
    def get_operational_status(self) -> Dict[str, Any]:
        ...
        for service_name, service in self.services.items():
            ...
            status["services"][service_name] = {
                ...
                "fallback_mode": self.fallbacks.get(service_name).fallback_mode if self.fallbacks.get(service_name) else "none"
            }
            
        # Determine overall health
        unhealthy_services = sum(1 for s in status["services"].values() if not s["healthy"])
    ...
    def create_fallback_storage(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Created fallback storage for {service_name}: {fallback.local_storage_path}")
            return True
        except Exception as e:
