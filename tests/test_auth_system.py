...
from nqba_stack.auth import (
    AuthManager, User, Role, Permission, UserSession,
    UserCreate, UserUpdate, LoginRequest, LoginResponse,
    UserStatus, RoleLevel, PermissionCategory
)
...
class TestAuthManager:
    ...
    def test_create_user_duplicate_username(self, auth_manager):
        ...
        success1, message1, user1 = auth_manager.create_user(user_data1, [founder_role.id])
        ...
        success2, message2, user2 = auth_manager.create_user(user_data2, [founder_role.id])
...
class TestAuthIntegration:
    ...
    def test_full_user_lifecycle(self, auth_manager):
        ...
        # 5. Verify update
        success, message, updated_user = auth_manager.get_user_by_id(user.id, [founder_role.id])
        assert success is True
        ...
        # 7. Verify deletion
        success, message, deleted_user = auth_manager.get_user_by_id(user.id, [founder_role.id])
        assert success is False
