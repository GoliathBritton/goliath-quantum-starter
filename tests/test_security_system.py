...
from src.nqba_stack.security.kms_manager import (
    KMSManager, KMSProvider, SecretMetadata, RotationPolicy
)
from src.nqba_stack.security.encryption_manager import (
    EncryptionManager, EncryptionLevel, EncryptionKey, TenantEncryptionConfig
)
from src.nqba_stack.security.iam_manager import (
    IAMManager, RoleLevel, Permission, Organization, User, APIKey
)
from src.nqba_stack.security.audit_logger import (
    AuditLogger, AuditEventType, AuditSeverity, AuditEvent, AuditLogEntry
)
from src.nqba_stack.security.compliance_manager import (
    ComplianceManager, ComplianceFramework, ControlCategory, DataSubjectType, DataProcessingPurpose
)
...
class TestKMSManager:
    ...
    def test_create_secret(self):
        ...
        secret_id = asyncio.run(self.kms_manager.create_secret(
            name="test_secret",
            value="test_value",
...
class TestEncryptionManager:
    ...
    def test_encrypt_decrypt_data(self):
        ...
        # Decrypt data
        decrypted_data = self.encryption_manager.decrypt_data(encrypted_package, tenant_id)
    ...
    def test_field_level_encryption(self):
        ...
        # Encrypt PII fields
        ssn_encrypted = self.encryption_manager.encrypt_field("123-45-6789", tenant_id, "ssn")
        cc_encrypted = self.encryption_manager.encrypt_field("4111-1111-1111-1111", tenant_id, "credit_card")
...
class TestIAMManager:
    ...
    def test_permission_management(self):
        ...
        # Check permissions
        can_read = self.iam_manager.check_permission(user_result["user_id"], Permission.DATA_READ)
        can_write = self.iam_manager.check_permission(user_result["user_id"], Permission.DATA_WRITE)
        
        assert can_read is True
        assert can_write is False
        
        # Update role to analyst
        success = self.iam_manager.update_user_role(user_result["user_id"], RoleLevel.ANALYST)
        assert success is True
        
        # Check updated permissions
        can_write_after = self.iam_manager.check_permission(user_result["user_id"], Permission.DATA_WRITE)
...
class TestSecurityIntegration:
    ...
    def test_end_to_end_security_workflow(self):
        ...
        assert self.encryption_manager.get_tenant_status(tenant_id)["tenant_id"] == tenant_id
