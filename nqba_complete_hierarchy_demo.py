...
async def demo_qhc_decision_making(qhc):
    ...
    # Import QHC types
    from src.nqba_stack.core.quantum_high_council import QHCBusinessUnit, QHCDecisionType
...
async def demo_quantum_digital_agents(digital_agents):
    ...
    operations = [
        (DigitalOperationType.STRATEGIC_PLANNING, QHCBusinessUnit.FLYFOX_AI, 
         "Develop quantum-enhanced energy management strategy"),
        (DigitalOperationType.INNOVATION_ACCELERATION, QHCBusinessUnit.GOLIATH_TRADE,
         "Accelerate DeFi innovation pipeline"),
        (DigitalOperationType.OPERATIONAL_EXCELLENCE, QHCBusinessUnit.SIGMA_SELECT,
...
async def demo_cross_layer_integration(qhc, digital_agents, qsai_engine, qea_do):
    ...
    print(f"   📊 Requires Human Approval: {strategic_decision.requires_human_approval}")
...
async def demo_automation_metrics(qhc, digital_agents):
    ...
    overall_automation = (qhc_metrics['overall_automation'] + digital_metrics['overall_automation']) / 2
...
async def demo_business_unit_operations(qhc, digital_agents):
    ...
    for business_unit, description in business_units:
        ...
        unit_decisions = [d for d in qhc.decisions.values() if d.business_unit == business_unit]
        ...
        unit_operations = [op for op in digital_agents.operations.values() if op.business_unit == business_unit]
        ...
        if unit_member:
            ...
            print(f"   🤖 Automation Capabilities: {len(unit_member.automation_capabilities)} areas")
