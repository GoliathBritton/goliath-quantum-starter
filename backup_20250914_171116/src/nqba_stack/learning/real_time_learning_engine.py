...
class RealTimeLearningEngine:
    ...
    async def add_learning_example(
        ...
        try:
            ...
            if (
                self.auto_learning_enabled
                and len(self.learning_examples[tenant_id])
                >= self.min_examples_for_learning
            ):
    ...
    async def _trigger_learning(self, model_id: str, tenant_id: str) -> None:
        ...
        try:
            ...
            # Get recent examples
            recent_examples = self.learning_examples[tenant_id][
                -self.min_examples_for_learning :
            ]
            ...
            if learning_result.improvement_score > self.learning_threshold:
                ...
                if len(self.learning_history[model_id]) > self.max_model_versions:
                    self.learning_history[model_id] = self.learning_history[model_id][
                        -self.max_model_versions :
                    ]

                logger.info(
                    f"Model {model_id} improved by {learning_result.improvement_score:.2%}"
                )
    ...
    async def _perform_learning(
        self, model: LearningModel, examples: List[LearningExample]
    ) -> LearningResult:
        ...
        if successful_examples and unsuccessful_examples:
            ...
            if "num_reads" in current_params:
                current_params["num_reads"] = min(
                    current_params["num_reads"] * 1.1, 10000  # Cap at 10k
                )

            if "annealing_time" in current_params:
                current_params["annealing_time"] = min(
                    current_params["annealing_time"] * 1.05, 1000  # Cap at 1000ms
                )
        ...
        if violation_examples:
            ...
            if "evolution_rate" in current_params:
                current_params["evolution_rate"] = min(
                    current_params["evolution_rate"] * 1.2, 0.5  # Cap at 50%
                )

            if "adaptation_threshold" in current_params:
                current_params["adaptation_threshold"] = max(
                    current_params["adaptation_threshold"] * 0.9, 0.1  # Floor at 10%
                )
    ...
    async def _learn_resource_allocation(
        ...
        high_performance_examples = [
            ex for ex in examples if ex.performance_score > 0.8
        ]
        ...
        if high_performance_examples:
            ...
            if "resource_multiplier" in current_params:
                current_params["resource_multiplier"] = min(
                    current_params["resource_multiplier"] * 1.05, 2.0  # Cap at 2x
                )

            if "scaling_factor" in current_params:
                current_params["scaling_factor"] = min(
                    current_params["scaling_factor"] * 1.1, 1.5  # Cap at 1.5x
                )
        ...
        try:
            ...
            for key in expected_output:
                if key in actual_output:
                    if isinstance(expected_output[key], (int, float)) and isinstance(
                        actual_output[key], (int, float)
                    ):
                        # Numeric similarity
                        ...
                        if expected_val != 0:
                            similarity = 1.0 - min(
                                abs(actual_val - expected_val) / abs(expected_val), 1.0
                            )
                            score += similarity
                            count += 1
                        else:
    ...
    async def get_tenant_learning_summary(self, tenant_id: str) -> Dict[str, Any]:
        ...
        tenant_models = [
            m
            for m in self.learning_models.values()
            if m.model_id.startswith(f"model_{tenant_id}")
        ]
    ...
    async def load_model(
        ...
        try:
            if version is None:
                ...
                # Get latest version
                latest_file = max(
                    model_files, key=lambda x: int(x.stem.split("_v")[-1])
                )
                ...
                if not model_file.exists():
                    raise ValueError(
                        f"Model version {version} not found for {model_id}"
                    )
    ...
    async def export_learning_data(
        self, tenant_id: str, format: str = "json"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_examples = (
                tenant_examples[-100:]
                if len(tenant_examples) > 100
                else tenant_examples
            )
