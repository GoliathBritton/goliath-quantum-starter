...
class KMSManager:
    ...
    def _setup_azure_keyvault(self):
        ...
        try:
            ...
            logger.warning(
                "azure-keyvault-secrets not available, falling back to local KMS"
            )
            self.provider = KMSProvider.LOCAL
            self._setup_local_kms()
    ...
    def _setup_local_kms(self):
        ...
        if not self.master_key:
            ...
            logger.warning(
                "Generated new local master key - not suitable for production"
            )
    ...
    async def create_secret(
        ...
        metadata = SecretMetadata(
            ...
            + timedelta(days=rotation_policy.rotation_interval_days),
