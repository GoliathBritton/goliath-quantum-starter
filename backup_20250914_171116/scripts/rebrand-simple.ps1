...
# Replace Oracle references with Quantum Nexus branding
...
$BRAND_MAPPINGS = @(
    @{"old" = "Oracle AI"; "new" = "Quantum Nexus AI"},
    @{"old" = "Oracle Platform"; "new" = "Quantum Nexus Platform"},
    @{"old" = "Oracle API"; "new" = "Quantum Nexus API"},
    @{"old" = "Oracle Agent"; "new" = "Quantum Nexus Agent"},
    @{"old" = "Oracle Quantum"; "new" = "Quantum Nexus"},
    @{"old" = "oracle_ai"; "new" = "quantum_nexus"},
    @{"old" = "oracle-ai"; "new" = "quantum-nexus"},
    @{"old" = "oracle_"; "new" = "quantum_nexus_"},
    @{"old" = "oracle-"; "new" = "quantum-nexus-"},
    @{"old" = "oracle.ai"; "new" = "quantum-nexus.ai"},
    @{"old" = "OracleAI"; "new" = "QuantumNexus"},
    @{"old" = "Oracle"; "new" = "Quantum Nexus"},
    @{"old" = "oracle"; "new" = "quantum-nexus"},
    @{"old" = "ORACLE"; "new" = "QUANTUM_NEXUS"}
)
...
function Rename-OracleFiles {
    Write-Log "Scanning for Oracle-named files and directories..."
    
    # Find files with Oracle in the name
    $oracleFiles = Get-ChildItem -Path $PROJECT_ROOT -Recurse -File | Where-Object {
        $_.Name -like "*oracle*" -or $_.Name -like "*Oracle*"
    }
    
    # Find directories with Oracle in the name
    $oracleDirs = Get-ChildItem -Path $PROJECT_ROOT -Recurse -Directory | Where-Object {
        $_.Name -like "*oracle*" -or $_.Name -like "*Oracle*"
    }
    ...
    $sortedDirs = $oracleDirs | Sort-Object { $_.FullName.Split([IO.Path]::DirectorySeparatorChar).Count } -Descending
...
function Update-PackageFiles {
    ...
    if (Test-Path $packageJsonPath) {
        try {
            ...
            if ($packageJson.name -and $packageJson.name -like "*oracle*") {
                $packageJson.name = $packageJson.name -replace "oracle", "quantum-nexus"
            }
            
            if ($packageJson.description -and $packageJson.description -like "*Oracle*") {
                foreach ($mapping in $BRAND_MAPPINGS) {
...
function Invoke-Rebranding {
    ...
    if (-not $Force -and -not $DryRun) {
        $confirmation = Read-Host "This will replace all Oracle references with Quantum Nexus. Continue? (y/N)"
    ...
    # Rename Oracle files and directories
    Rename-OracleFiles
    ...
    if ($DryRun) {
        ...
        Write-Log "Oracle references have been replaced with Quantum Nexus branding."
    }
}
...
function Show-BrandingStatus {
    ...
    foreach ($file in $filesToCheck) {
        try {
            ...
            # Count Oracle references
            $oracleMatches = ([regex]"Oracle").Matches($content).Count
            $oracleReferences += $oracleMatches
            
            # Count Quantum Nexus references
            $quantumMatches = ([regex]"Quantum Nexus").Matches($content).Count
            $quantumReferences += $quantumMatches
            
            if ($oracleMatches -gt 0) {
                Write-Log "Oracle references found in: $($file.FullName) ($oracleMatches occurrences)" "WARN"
            }
        }
        catch {
    ...
    Write-Log "  Oracle references: $oracleReferences"
    Write-Log "  Quantum Nexus references: $quantumReferences"
    
    if ($oracleReferences -eq 0) {
        Write-Log "Rebranding appears to be complete!" "SUCCESS"
    } else {
        Write-Log "$oracleReferences Oracle references still found" "WARN"
    }
}
...
switch ($Action.ToLower()) {
    ...
    "help" {
        ...
        Write-Host ""
        Write-Host "Actions:"
        Write-Host "  rebrand     Complete rebranding from Oracle to Quantum Nexus"
