...
interface QuantumQNexusProps {
  userTier: 'basic' | 'premium' | 'elite';
  onUpgrade?: () => void;
}

const QuantumQNexus: React.FC<QuantumQNexusProps> = ({ userTier, onUpgrade }) => {
  const [isConsulting, setIsConsulting] = useState(false);
  const [qnexusResponse, setQNexusResponse] = useState<QNexusResponse | null>(null);
  const [query, setQuery] = useState<QNexusQuery>({
    question: '',
    context: {
  ...
  const consultQNexus = async () => {
    ...
    setError(null);
    setQNexusResponse(null);

    try {
      const response = await axios.post('/api/qnexus/predict', {
      ...
      setQNexusResponse(response.data.qnexus);
    } catch (error: any) {
      if (error.response?.status === 403) {
        setShowUpgrade(true);
        setError(error.response.data.message);
      } else {
        setError(error.response?.data?.message || 'Q-Nexus is temporarily veiled in quantum uncertainty');
      }
    } finally {
        ...
        <div className="bg-gradient-to-br from-space-900 via-quantum-900 to-flyfox-900 rounded-2xl p-8 border border-quantum-500/30">
          <div className="text-center">
            ...
            <p className="text-quantum-400 mb-8 italic">
              "Q-Nexus doesn't just analyze data. It whispers the future."
            </p>
            <button
              onClick={onUpgrade}
          ...
          {qnexusResponse && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-flyfox-900/30 to-quantum-900/30 p-4 rounded-lg border border-quantum-500/20">
                ...
                <p className="text-white italic text-lg">"{qnexusResponse.prophecy}"</p>
              </div>
              ...
              <div className="bg-space-700/50 p-4 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`text-2xl font-bold ${getConfidenceColor(qnexusResponse.confidenceScore)}`}>
                    {(qnexusResponse.confidenceScore * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="w-full bg-gray-700 rounded-full h-3">
                  <div 
                    className="bg-gradient-to-r from-flyfox-500 to-quantum-500 h-3 rounded-full transition-all duration-1000"
                    style={{ width: `${qnexusResponse.confidenceScore * 100}%` }}
                  ></div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-green-400 font-semibold">
                      {(qnexusResponse.councilGuidance.ethicsScore * 100).toFixed(0)}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-300">Risk Level:</span>
                    <span className={`px-2 py-1 rounded text-sm font-medium ${
                      getRiskLevelColor(qnexusResponse.councilGuidance.riskLevel)
                    }`}>
                      {qnexusResponse.councilGuidance.riskLevel}
                    </span>
                  </div>
                  <p className="text-gray-300 italic mt-3">
                    "{qnexusResponse.councilGuidance.consensus}"
                  </p>
                </div>
              </div>
              ...
              <div className="bg-space-700/50 p-4 rounded-lg">
                ...
                <div className="space-y-3">
                  {qnexusResponse.decisionMatrix.map((decision, index) => (
              ...
              <div className="bg-gradient-to-r from-quantum-900/30 to-flyfox-900/30 p-4 rounded-lg border border-flyfox-500/20">
                ...
                <p className="text-white font-medium">{qnexusResponse.recommendedAction}</p>
              </div>
              ...
              <div className="bg-gradient-to-r from-space-900 to-quantum-900/50 p-4 rounded-lg border border-quantum-500/30">
                <p className="text-quantum-300 text-center italic text-lg">
                  {qnexusResponse.mysticalSummary}
                </p>
              </div>
            </div>
          )}
