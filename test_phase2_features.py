...
class Phase2Tester:
    ...
    async def test_multi_tenant(self) -> Dict[str, Any]:
        ...
        response = requests.post(
            f"{self.api_base_url}/phase2/tenant/create", json=tenant_data
        )

        if response.status_code != 200:
            raise Exception(
                f"Tenant creation failed: {response.status_code} - {response.text}"
            )
    ...
    async def test_real_time_learning(self) -> Dict[str, Any]:
        ...
        algorithm_data = {
            ...
            "hyperparameters": {"alpha": 0.1, "beta": 0.9, "max_iterations": 1000},
            "constraints": {"max_execution_time": 300, "memory_limit": "4GB"},
            "version": "2.1.0",
        }
        ...
        response = requests.get(f"{self.api_base_url}/phase2/analytics/learning")
        ...
        print(
            f"✅ Learning analytics retrieved: {learning_analytics['engine_metrics']['total_algorithms']} algorithms"
        )

        return {
            "status": "PASSED",
            "algorithm_id": self.algorithm_id,
            "total_algorithms": learning_analytics["engine_metrics"][
                "total_algorithms"
            ],
            "total_performance_records": learning_analytics["engine_metrics"][
    ...
    async def test_advanced_qubo(self) -> Dict[str, Any]:
        ...
        qubo_data = {
            ...
            "variable_names": [
                f"x_{i}_{j}_{k}" for i in range(8) for j in range(12) for k in range(6)
            ],
        ...
        response = requests.post(
            f"{self.api_base_url}/phase2/advanced-qubo/create", json=qubo_data
        )
        ...
        print(
            f"✅ QUBO optimization completed: {optimization_result['objective_value']}"
        )

        # Test QUBO analytics
        print("Testing QUBO analytics...")
        response = requests.get(f"{self.api_base_url}/phase2/analytics/qubo")
        ...
        print(
            f"✅ QUBO analytics retrieved: {qubo_analytics['optimization_metrics']['total_optimizations']} optimizations"
        )
        ...
        return {
            ...
            "total_optimizations": qubo_analytics["optimization_metrics"][
                "total_optimizations"
            ],
    ...
    async def test_performance_dashboard(self) -> Dict[str, Any]:
        ...
        response = requests.post(f"{self.api_base_url}/phase2/dashboard/start")
        ...
        response = requests.get(f"{self.api_base_url}/phase2/dashboard/summary")
        ...
        print(
            f"✅ Dashboard summary retrieved: {dashboard_summary['system_health']['overall_health']}"
        )
        ...
        response = requests.post(
            f"{self.api_base_url}/phase2/dashboard/record-metrics", json=system_metrics
        )

        if response.status_code != 200:
            print(
                f"⚠️ Performance metrics recording failed: {response.status_code} (this endpoint may not exist yet)"
            )
        else:
    ...
    async def test_integration_workflows(self) -> Dict[str, Any]:
        ...
        response = requests.get(f"{self.api_base_url}/phase2/analytics/tenant")

        if response.status_code == 200:
            tenant_system_analytics = response.json()
            workflow_steps.append(
                f"✅ Tenant system analytics: {tenant_system_analytics['system_overview']['active_tenants']} tenants"
            )
        else:
        ...
        if self.tenant_id and self.algorithm_id:
            ...
            if response.status_code == 200:
                workflow_steps.append(
                    "✅ Cross-component performance recording successful"
                )
            else:
    ...
    async def test_performance_scalability(self) -> Dict[str, Any]:
        ...
        print(
            f"✅ Concurrent requests: {successful_requests}/10 successful in {total_time:.2f}s"
        )
...
async def main():
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print(
                "python -m uvicorn src.nqba_stack.api_server:app --host 0.0.0.0 --port 8002 --reload"
            )
            return
