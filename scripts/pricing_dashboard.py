...
def ai_solutions_builder(lang):
    ...
    st.markdown(
        """Design, test, and deploy custom AI-powered pricing or business solutions.\n
    - Use OpenAI or custom models\n    - Integrate with your data\n    - Deploy as API or dashboard widget\n    """
        if lang == "en"
        else """Diseña, prueba y lanza soluciones de IA personalizadas para precios o negocio.\n- Usa OpenAI o modelos propios\n- Integra tus datos\n- Despliega como API o widget\n"""
    )
    prompt = st.text_area(
        "Describe your AI solution (e.g., 'Suggest optimal plan for user X')",
        key="ai_prompt",
    )
...
def contact_sales_form(lang):
    ...
    with st.form("contact_sales_form"):
        ...
        email = st.text_input(
            "Your Email" if lang == "en" else "Tu Correo", key="email"
        )
        ...
        if submitted:
            try:
                msg = EmailMessage()
                msg["Subject"] = (
                    f"[{BRANDING['goliath']['name']} | {BRANDING['flyfox']['name']} | {BRANDING['sigma_select']['name']}] Sales Inquiry from {name}"
                )
                msg["From"] = email
                ...
                st.success(
                    ...
                    if lang == "en"
                    else "¡Gracias! Nuestro equipo te contactará pronto."
                )
            except Exception:
                st.warning(
                    "Message not sent (demo mode)"
                    if lang == "en"
                    else "Mensaje no enviado (modo demo)"
                )
...
def main():
    ...
    st.session_state["role"] = st.sidebar.selectbox(
        "Role", ["client", "admin", "sales", "investor"], index=0
    )
    ...
    if st.sidebar.button("Live Chat" if lang == "en" else "Chat en Vivo"):
        streamlit_chat.chat(
            "How can we help you?" if lang == "en" else "¿En qué podemos ayudarte?"
        )
    ...
    st.markdown(
        '<div role="main" aria-label="Pricing Dashboard">', unsafe_allow_html=True
    )
    ...
    lang = st.sidebar.selectbox(
        "Language", ["en", "es"], index=0, help="Select language / Selecciona idioma"
    )
    ...
    st.header(
        "💎 Quantum Luxury Tiers" if lang == "en" else "💎 Niveles de Lujo Cuántico"
    )
    tiers = data["tiers"]
    ...
    for tier in tiers:
        if tier["name"] not in selected:
            continue
        if search and not (
            search.lower() in tier["name"].lower()
            or search.lower() in tier["desc"].lower()
            or any(search.lower() in f.lower() for f in tier["features"])
        ):
            continue
            ...
            [
                {
                    ...
                    **{t["name"]: t["price"] for t in h["pricing"].get("tiers", [])},
                }
                for h in history
            ]
        )
    ...
    if st.button("Export CSV"):
        st.download_button(
            "Download CSV", df.to_csv(index=False), file_name="pricing_history.csv"
        )
    ...
    st.header(
        data["investors"]["title"] if lang == "en" else "👑 Acceso para Inversores"
    )
    ...
    if st.sidebar.button("Export PDF"):
        st.info(
            "[TODO] PDF export coming soon. Use browser print to PDF as a workaround."
        )
